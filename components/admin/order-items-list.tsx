        ...
        return (
          <div key={`${item.id}-${idx}`} className="rounded-2xl border border-[#FAC1B5]/20 bg-[#FFFBF8] p-4 space-y-3">
            ...
            <div className="grid gap-1.5 text-sm text-[#98898D]">
              ...
              {!isCustom && <Row label="Unit price" value={`Rs. ${item.pricePerItem.toLocaleString()}`} />}
              ...
              {item.referenceImage && (
                <div className="flex flex-col gap-2 mt-2">
                  <span>{isCustom ? 'Reference Image' : 'Reference'}</span>
                  {item.referenceImage.startsWith('http') ? (
                    <a href={item.referenceImage} target="_blank" rel="noreferrer" className="inline-block">
                      <img 
                        src={item.referenceImage} 
                        alt="Reference" 
                        className="max-h-40 rounded-lg object-cover border border-[#FAC1B5]/30 hover:opacity-90 transition-opacity shadow-sm"
                      />
                    </a>
                  ) : (
                    <span className="text-[#2C2C2C]">{item.referenceImage}</span>
                  )}
                </div>
              )}
